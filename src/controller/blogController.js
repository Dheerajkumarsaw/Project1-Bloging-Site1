...
const blogCreate = async function (req, res) {
    try {
        ...
        // validations
        if (!isValid(title)) {
            return res.status(400).send({ status: false, message: "Please Enter Blogs title" });
        }
        if (!isValid(body)) {
            return res.status(400).send({ status: false, message: "Enter Blogs Details" });
        }
        if (!isValid(authorId)) {
            return res.status(400).send({ status: false, message: "Please Enter AuthorId" });
        }
        if (!isValidObjectId(authorId)) {
            return res.status(400).send({ status: false, message: `${authorId} is Not Valid AuthorId` });
        }
        if (!isValid(tags)) {
            return res.status(400).send({ status: false, message: "Enter Your Blog  Tags" });
        }
        if (!isValid(category)) {
            return res.status(400).send({ status: false, message: "Enter Blog category" });
        }
        if (!isValid(subcategory)) {
            return res.status(400).send({ status: false, message: "Enter Blog's subcategory" });
        ...
        if (!author) {
            return res.status(404).send({ status: false, message: `No auther exist with ${authorId} id Please register first` })
        }
...
const updateBlog = async function (req, res) {
    try {
        ...
        if (Object.keys(requestBody).length == 0) {
            return res.status(400).send({ status: false, message: "Please Enter Filter as you want" });
        }
        ...
        if (!isValid(body)) {
            return res.status(400).send({ status: false, message: "Enter body" })
        }
        if (!isValid(tags)) {
            return res.status(400).send({ status: false, message: "Enter tags" })
        }
        if (!isValid(category)) {
        ...
        if (!blog) {
            return res.status(404).send({ status: false, message: "no such blog exist with this id" });
        }
        let isData = await blogModel.findOne({ _id: blogId, isDeleted: false, isPublished: false });
        if (!isData) {
            return res.status(400).send({ status: false, message: "Allready Updated" })
...
let deleted = async function (req, res) {
    try {
        ...
        if (!isValidObjectId(blogid)) {
            return res.status(400).send({ status: false, message: `${blogid} is not valid id` });
        }
        ...
        await blogModel.findByIdAndUpdate({ _id: blogid, isDeleted: false }, { $set: { isDeleted: true, deleteAt: Date.now() } }, { new: true })
...
let deletequery = async function (req, res) {
    try {
        ...
        // validations
        if (!isValid(authorId)) {
            return res.status(400).send({ status: false, message: "Not valid Author id" });
        }
        if (!isValidObjectId(authorId)) {
            return res.status(400).send({ status: false, message: "Not valid Author id" })
        }
        ...
        if (!isValid(tags)) {
            return res.status(400).send({ status: false, message: "Enter Blog tags" });
        }
        if (!isValid(subcategory)) {
            return res.status(400).send({ status: false, message: "Enter Blog subcategory" });
        }

        const filter = { data, isPublished: false }
        deletedata = await blogModel.find(filter)
        if (!deletedata) {
            return res.status(404).send({ status: false, message: "Such Blog not found" })
        }
